...
#include "tactic/str/ext_strSimplify_tactic.h"
...
class ext_strSimplify_tactic : public tactic {
    struct imp {
        ...
        void process_eq(expr* eq, goal_ref const& g, expr_substitution& sub) {
            ...
            // Rewrite: (= (str.indexof H N 0) -1) --> not(str.contains H N)
            {
                ...
                if (rewrite_applies) {
                    TRACE("ext_strSimplify_tactic", tout << "str.indexof = -1 rewrite applies: " << mk_pp(haystack, m) << " does not contain " << mk_pp(needle, m) << std::endl;);
            ...
            // Rewrite: (<= (str.to.int S) #const) and #const is not too large ==> (str.to.int S) = -1 OR (str.in_re S (0* ++ (0, 1, 2, ..., #const)))
            {
                ...
                if (m_autil.is_numeral(rhs, integer_constant) && integer_constant.is_nonneg() && integer_constant <= str_to_int_finitization_upper_bound) {
                    if (u.str.is_stoi(lhs, string_subterm)) {
                        TRACE("ext_strSimplify_tactic", tout << "str.to_int finitization applies: " << mk_pp(rhs, m) << " <= " << integer_constant << std::endl;);
                        ...
                        TRACE("ext_strSimplify_tactic", tout << mk_pp(subst, m) << std::endl;);
                        sub.insert(le, subst);
                    }
                }
            }
        ...
        void process_ge(expr* ge, goal_ref const& g, expr_substitution& sub) {
            ...
            // Rewrite: (str.indexof H N 0) >= 0 --> (str.contains H N)
            {
                ...
                if (rewrite_applies) {
                    TRACE("ext_strSimplify_tactic", tout << "str.indexof >= 0 rewrite applies: " << mk_pp(haystack, m) << " contains " << mk_pp(needle, m) << std::endl;);
        ...
        void process_lt(expr* lt, goal_ref const& g, expr_substitution& sub) {
            ...
            // Rewrite: (< (str.to.int S) #const) and #const is not too large ==> (str.to.int S) = -1 OR (str.in_re S (0* ++ (0, 1, 2, ..., #const-1)))
            {
                ...
                if (m_autil.is_numeral(rhs, integer_constant) && integer_constant >= rational(1) && integer_constant <= str_to_int_finitization_upper_bound) {
                    if (u.str.is_stoi(lhs, string_subterm)) {
                        TRACE("ext_strSimplify_tactic", tout << "str.to_int finitization applies: " << mk_pp(rhs, m) << " <= " << integer_constant - 1 << std::endl;);
            ...
            // Rewrite: (str.indexof H N 0) < n < 0 --> not(str.contains H N)
            {
                ...
                if (rewrite_applies) {
                    TRACE("ext_strSimplify_tactic", tout << "str.indexof < negative rewrite applies: " << mk_pp(haystack, m) << " does not contain " << mk_pp(needle, m) << std::endl;);
                    expr_ref h_not_in_n(m.mk_not(u.str.mk_contains(haystack, needle)), m);
        ...
        void process_gt(expr* gt, goal_ref const& g, expr_substitution& sub) {
            ...
            // Rewrite: (str.indexof H N 0) > -1 --> (str.contains H N)
            {
                ...
                if (rewrite_applies) {
                    TRACE("ext_strSimplify_tactic", tout << "str.indexof > -1 rewrite applies: " << mk_pp(haystack, m) << " contains " << mk_pp(needle, m) << std::endl;);
                    expr_ref h_in_n(u.str.mk_contains(haystack, needle), m);
        ...
        void operator()(goal_ref const& g, goal_ref_buffer& result) {
            ...
            TRACE("ext_strSimplify_tactic", tout << "BEFORE:" << std::endl; g->display(tout););
            ...
            for (unsigned idx = 0; idx < size; ++idx) {
                ...
                if (is_app(curr)) {
                    ...
                    while (!stack.empty()) {
                        ...
                        TRACE("ext_strSimplify_tactic", tout << "curr: " << mk_pp(curr, m) << std::endl;);
            ...
            result.push_back(g.get());
            SASSERT(g->is_well_formed());
            TRACE("ext_strSimplify_tactic", tout << "AFTER: " << std::endl; g->display(tout););
...
public:
    ext_strSimplify_tactic(ast_manager& m, params_ref const& p) :
        m_params(p) {
        m_imp = alloc(imp, m, p);
    }

    tactic* translate(ast_manager& m) override {
        return alloc(ext_strSimplify_tactic, m, m_params);
    }

    ~ext_strSimplify_tactic() override {
        dealloc(m_imp);
    }
...
tactic * mk_ext_strSimplify_tactic(ast_manager & m, params_ref const & p) {
    return clean(alloc(ext_strSimplify_tactic, m, p));
}
